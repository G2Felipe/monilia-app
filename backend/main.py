...
MODEL_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "cacao_resnet101_classifier3.keras"))
CLASS_NAMES_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "class_names.json"))
...
def load_model_and_classes():
    ...
    print("Iniciando carga de modelo y clases...")
    ...
    try:
        print("Intentando cargar archivo de clases...")
        with open(CLASS_NAMES_PATH, 'r') as f:
            class_names = json.load(f)
            print("Archivo de clases cargado exitosamente")
    except FileNotFoundError as e:
        print(f"Error al cargar archivo de clases: {str(e)}")
        print(f"Ruta intentada: {CLASS_NAMES_PATH}")
        return False
    except Exception as e:
        print(f"Error inesperado al cargar clases: {str(e)}")
        return False
    ...
    try:
        ...
        print(f"Ruta intentada: {MODEL_PATH}")
        return False

# Cargar el modelo y las clases al inicio
if not load_model_and_classes():
    print("Error: No se pudo cargar el modelo o las clases")
else:
...
async def predict_image(file: UploadFile = File(...)):
    ...
    if model is None or class_names is None:
        raise HTTPException(
            status_code=503, 
            detail="El servicio no está listo. El modelo o las clases no están cargados."
        )
...
async def health_check():
    ...
    is_ready = model is not None and class_names is not None
    return {
